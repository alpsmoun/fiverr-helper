$(function () {
	...
	$(".filtering_form").submit(function () {
		var crud_page = parseInt(
			$(this).closest(".flexigrid").find(".crud_page").val(),
			10
		);
		var last_page = parseInt(
			$(this).closest(".flexigrid").find(".last-page-number").html(),
			10
		);
	...
	$(".prev-button").click(function () {
		if ($(this).closest(".flexigrid").find(".crud_page").val() != "1") {
			$(this)
				...
				.find(".crud_page")
				.val(
					parseInt($(this).closest(".flexigrid").find(".crud_page").val(), 10) -
						1
				);
	...
	$(".last-button").click(function () {
		$(this)
			.closest(".flexigrid")
			.find(".crud_page")
			.val($(this).closest(".flexigrid").find(".last-page-number").html());
		$(this).closest(".flexigrid").find(".filtering_form").trigger("submit");
	});
	...
	$(".next-button").click(function () {
		$(this)
			...
			.find(".crud_page")
			.val(
				parseInt($(this).closest(".flexigrid").find(".crud_page").val()) + 1
			);
		$(this).closest(".flexigrid").find(".crud_page").trigger("change");
	});
	...
	$(".export-anchor").click(function () {
		...
		var form_input_html = "";
		$.each(
			$(this).closest(".flexigrid").find(".filtering_form").serializeArray(),
			function (i, field) {
				form_input_html =
					form_input_html +
		...
		var form_on_demand = $("<form/>")
			.attr("id", "export_form")
			.attr("method", "post")
			.attr("target", "_blank")
			.attr("action", export_url)
			.html(form_input_html);

		$(this)
			.closest(".flexigrid")
			.find(".hidden-operations")
			.html(form_on_demand);
	...
	if ($(".flexigrid").length == 1) {
		...
		if (cookie_crud_page !== null && cookie_per_page !== null) {
			...
			if (cookie_search_text !== "") $("#quickSearchButton").trigger("click");
...
function displaying_and_pages(this_container) {
	...
	this_container
		...
		.html(Math.ceil(total_items / per_page));

	if (total_items == 0) {
		this_container.find(".page-starts-from").html("0");
	} else {
		this_container
			.find(".page-starts-from")
